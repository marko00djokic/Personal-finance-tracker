<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Nova planirana transakcija</h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-lg mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm rounded-lg p-6">
                ...
                <form method="POST" action="{{ route('planned-transactions.store') }}"
                      x-data="{ recurrence: '{{ old('recurrence_type', 'monthly') }}' }">
                    @csrf
                    ...
                    <div class="space-y-5">
                        ...
                        <div>
                            <label for="next_due_date" class="block text-sm font-medium text-gray-700 mb-1">Datum prvog dospeća</label>
                            <input type="date" id="next_due_date" name="next_due_date"
                                   value="{{ old('next_due_date', now()->format('Y-m-d')) }}"
                                   class="w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
                                   required>
                            @error('next_due_date')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div>
                            ...
                            <select id="recurrence_type" name="recurrence_type"
                                    ...
                                    class="w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
                                    required>
                                <option value="none"    {{ old('recurrence_type') === 'none'    ? 'selected' : '' }}>Jednokratno</option>
                                <option value="daily"   {{ old('recurrence_type') === 'daily'   ? 'selected' : '' }}>Dnevno</option>
                                <option value="weekly"  {{ old('recurrence_type') === 'weekly'  ? 'selected' : '' }}>Nedeljno</option>
                                <option value="monthly" {{ old('recurrence_type', 'monthly') === 'monthly' ? 'selected' : '' }}>Mesečno</option>
